...
class TaskCompletionService {
  ...
  void startAutoCompleteTimer() {
    ...
    debugPrint('⏰ Auto-complete timer started (runs every 10 minutes)');
  }
  ...
  Future<void> _checkAndAutoCompleteTasks() async {
    ...
    try {
      ...
      // Find tasks that need auto-completion
    ...
    switch (task.type) {
      ...
      // Classes: Auto-complete when endTime is passed
        if (task.endTime != null) {
          return now.isAfter(task.endTime!);
        }
        // If no endTime, check date
        final taskDate = DateTime(task.date.year, task.date.month, task.date.day);
        return now.isAfter(taskDate);

      case TaskType.exam:
      // Exam: Auto-complete when date is passed
        final examDateTime = task.startTime ?? task.date;
        return now.isAfter(examDateTime);
  ...
  Future<bool> _completeTask(Task task, {bool isAuto = false}) async {
    try {
      ...
      // ✅ Use TaskProvider for mutation
      Task updatedTask;
      if (isAuto) {
        // For auto-complete, we need to mark as auto-completed
        ...
        // Manual completion - use TaskProvider toggle
        updatedTask = await _taskProvider!.toggleTaskDone(task.id! as Task);
      }
  ...
  Future<bool> uncompleteTask(Task task) async {
    try {
      ...
      // ✅ Use TaskProvider to uncomplete the task
      await _taskProvider!.toggleTaskDone(task.id! as Task);
